    ...
    def create_bootable_iso(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                ...
                subprocess.check_call(['sudo', 'grub2-mkimage',
                                       '-O', 'i386-pc-eltorito',
                                       '-d', '/usr/lib/grub/i386-pc',
                                       '-p', '/boot/grub2',
                                       'iso9660', 'biosdisk', 'halt'])
                graft_path = '.=' + os.path.join(tmp_dir, "os").replace("\\", "\\\\").replace("=", "\\=")
                efiboot_img = os.path.join(tmp_dir, 'os/images/efiboot.img')
                with open('os/EFI/BOOT', mode=0o644, dir_fd=f) as g:
                    subprocess.check_call(['sudo', 'cat', '/boot/efi/EFI/qubes/grubx64.efi'], stdout=g)
                subprocess.check_call(['mkefiboot', '--label=ANACONDA',
